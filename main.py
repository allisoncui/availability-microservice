...
def fetch_available_days(restaurant_code, num_seats=2):
    today = datetime.now().strftime('%Y-%m-%d')
    end_date = (datetime.now() + timedelta(days=30)).strftime('%Y-%m-%d')
    url = 'https://api.resy.com/4/venue/calendar'
    params = {
        'venue_id': restaurant_code,
        'num_seats': num_seats,
        'start_date': today,
        'end_date': end_date
    }
...
# Fetch available times for a specific day
def fetch_available_times(restaurant_code, num_seats=2, day=None):
...
# Check availability for a restaurant code
                ...
                if available_slots and 'results' in available_slots:
                    ...
                    for venue in venues:
                        for slot in venue.get('slots', []):
                            ...
                            if start_time:
                                logger.info(
                                    f"First available reservation for {restaurant_code} on {day['date']} at {start_time}")
                                return {
...
def check_availability_task(restaurant_code, request_id, callback_url=None):
    result = check_availability(restaurant_code)
    if result:
        availability_results[restaurant_code] = result
        logger.info(f"Stored first available reservation for {restaurant_code}: {result}")
    else:
    ...
    if callback_url:
        try:
            response = requests.post(callback_url, json={"status": "complete", "data": result})
            response.raise_for_status()
            logger.info(f"Callback to {callback_url} successful.")
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to send callback: {e}")

# Endpoint to initiate availability check
...
async def check_status(request_id: str):
    if request_id in task_status:
        ...
        if status == "complete":
            return {"status": "complete", "data": availability_results.get(request_id)}
        else:
            return {"status": "processing", "data": None}
    else:
        raise HTTPException(status_code=404, detail="Request ID not found")

# Endpoint to get the first available reservation by restaurant code
