...
# Fetch available days from Resy API
...
# Fetch available times from Resy API
...
# Endpoint to fetch availability via GET (ping Resy API directly)
@app.get("/availability/{restaurant_code}")
async def get_availability(restaurant_code: str):
    try:
        num_seats = 2
        available_days = fetch_available_days(restaurant_code, num_seats)
        if available_days and 'scheduled' in available_days:
            available_times = []
            for day in available_days['scheduled']:
                if day['inventory']['reservation'] == 'available':
                    day_date = day['date']
                    times = fetch_available_times(restaurant_code, num_seats, day_date)
                    available_times.append({day_date: times})
            return {"restaurant_code": restaurant_code, "available_times": available_times}
        return {"message": "No availability found for the restaurant"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching availability: {e}")

# Background task to check availability (POST)
def check_availability_task(restaurant_code, request_id, callback_url=None):
    # Perform availability check (e.g., database operations)
    availability_results[request_id] = {"availability": "Some data"}  # Example data
    task_status[request_id] = "complete"

    # If a callback URL is provided, post the result to it
    if callback_url:
        try:
            response = requests.post(callback_url, json={"status": "complete", "data": availability_results[request_id]})
            response.raise_for_status()
            print(f"Callback to {callback_url} successful.")
        except requests.exceptions.RequestException as e:
            print(f"Failed to send callback: {e}")

# POST to initiate the background task for availability check
@app.post("/availability/{restaurant_code}")
async def post_availability_check(restaurant_code: str, request: Request, background_tasks: BackgroundTasks):
...
# Endpoint to check the status of the background task (polling)
